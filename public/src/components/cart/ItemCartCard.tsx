...
export const ItemCartCard = ({ item }: { item: CartItem }) => {
    const { updateQuantity, removeItem } = useCart();

    return (
        <div className="flex items-center justify-between p-2 border rounded-3xl bg-card shadow-sm w-5xl">
            <div className="flex items-center gap-4">
                <img src={item.image} alt={item.name} className="w-30 h-30 rounded-2xl object-cover object-[40%_40%]" />
                <div>
                    <h3 className="text-lg font-semibold text-foreground">{item.name}</h3>
                    <p className="text-sm text-muted-foreground">{formatters.formatCurrency(item.price)} / unidade</p>
